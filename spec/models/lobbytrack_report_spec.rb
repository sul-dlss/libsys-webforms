...
RSpec.describe LobbytrackReport, type: :model do
  ...
  describe 'sql query for visits' do
    ...
    it 'creates a querystring with the given lobbytrack id' do
      expect(described_class.visits('1234567')).to eq "#{default_sets} DECLARE @id VARCHAR(7)" \
  ...
  describe 'sql query for visits_for_dates' do
    it 'creates a querystring with the given dates' do
      expect(described_class.visits_for_dates('01/01/2019', '01/01/2020')).to eq "#{default_sets} DECLARE @date1 date" \
  ...
  describe 'sql query for checkins' do
    it 'creates a querystring with the given lobbytrack id' do
      expect(described_class.checkins('1234567')).to eq "#{default_sets} DECLARE @id VARCHAR(7)" \
  ...
  describe 'sql query for visitor' do
    it 'creates a querystring for visitor info' do
      expect(described_class.visitor_data('1234567')).to eq "#{default_sets} DECLARE @id VARCHAR(7)" \
