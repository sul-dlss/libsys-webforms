...
describe 'Endowed Funds Reports Page', js: true do
  before do
  ...
  it 'renders the hidden field for date_request' do
    attribute = page.find_by_id('endowed_funds_report_date_request', visible: false).value
  ...
  describe 'when too many finds are selected' do
    before do
      page.find(:css, 'input#endowed_funds_report_fund_select_fund_name_id').click
