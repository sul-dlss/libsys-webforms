...
RSpec.describe AuthorizedUsersController, type: :controller do
  ...
  describe 'get#edit' do
    it 'renders the correct template' do
      get 'edit', user_id: @authorized_user
      expect(response).to render_template('edit')
    end
  end
  describe 'patch#update' do
    it 'does not updates the user id' do
      patch :update, user_id: 'admin_user', authorized_user: { user_id: 'adminuser' }
      @authorized_user.reload
      expect(@authorized_user.user_id).to eq('admin_user')
    end
    it 'does not update if user name is blank' do
      patch :update, user_id: 'admin_user', authorized_user: { user_name: '' }
      @authorized_user.reload
      expect(@authorized_user.user_name).to eq('Admin User')
      expect(flash[:error]).to be_present
    end
    it 'updates an checked checkbox to Y' do
      patch :update, user_id: 'admin_user', authorized_user: { unicorn_circ_batch: 'Y' }
      @authorized_user.reload
      expect(@authorized_user.unicorn_circ_batch).to eq('Y')
    end
    it 'updates an unchecked checkbox to nil' do
      patch :update, user_id: 'admin_user', authorized_user: { ora_admin: '' }
      @authorized_user.reload
