...
class EndowedFundsReport
  ...
  def self.ol_cat_key(fund, date_start, date_end)
    if fund.is_a?(Array)
      fund_codes = []
      fund.each do |fc|
        Expenditures.where("ta_fund_code = ? AND ta_date_2encina between TO_DATE(?, 'yyyy-mm-dd') AND TO_DATE(?, 'yyyy-mm-dd')",
      ...
      Expenditures.where("ta_fund_code LIKE ? AND ta_date_2encina between TO_DATE(?, 'yyyy-mm-dd') AND TO_DATE(?, 'yyyy-mm-dd')",
